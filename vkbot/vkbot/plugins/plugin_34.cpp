...
void Rule34Plugin::exec(const std::vector<std::string>& args, long peer_id, long from_id) {
	...
	auto ret = http::get("https://rule34.xxx/index.php?page=dapi&q=index&s=post&tags=" + black_list + join(std::vector<std::string>(args.begin()+offset, args.end()), "+") + "&limit=10000");
